...
## Sprint 55 — Production Semantic Retrieval

### Task 1 — Production Semantic Retrieval

- Status: Done
- Summary: Reviewed the existing wiring first (same discipline as Sprints 52-54) and found "replace only the repository implementation" already true, from an earlier Sprint: `application_context._build_book_embedding_repository()` already switches to `PostgreSQLBookEmbeddingRepository` when `BOOK_REPOSITORY_BACKEND=postgresql`, and `SemanticRecommendationEngine`/`HybridRecommendationEngine` are constructed identically regardless of backend -- they depend only on the `BookEmbeddingRepository` port, never on a concrete adapter. `tests/infrastructure/test_postgresql_book_embedding_repository.py` already had two tests proving semantic and hybrid recommendations work correctly against a real PostgreSQL-backed `ApplicationContext` (at the use-case layer). This Sprint's actual work was extending that proof to the REST layer and to a direct in-memory/PostgreSQL comparison, plus the explicit validation activities this Sprint's requirements ask for.
  - **`tests/api/test_semantic_recommendation_via_postgresql.py`** (new, 2 tests): the real FastAPI app, served through `TestClient`, backed by `PostgreSQLBookRepository` + `PostgreSQLBookEmbeddingRepository` instead of the in-memory defaults used by every other API test in this suite. Confirms the existing empty-embedding fallback (200, `{"items": []}`) holds under PostgreSQL exactly as under in-memory, and that `GET /recommendations/semantic`/`GET /recommendations/hybrid` return the correct, correctly-shaped results end-to-end -- "preserve API behaviour" verified at the actual API layer, not inferred from use-case-level tests alone.
  - **`tests/infrastructure/test_postgresql_book_embedding_repository.py`** (+1 test): `test_in_memory_and_postgresql_repositories_rank_identically` -- saves the same 5 embeddings into both `InMemoryBookEmbeddingRepository` and `PostgreSQLBookEmbeddingRepository` and asserts `find_similar()` returns the identical book-id ordering from both. This is "compare retrieval quality" made concrete: both adapters implement the same port with the same cosine-similarity contract, so PostgreSQL's HNSW-indexed approximate search costs no relevance a caller would ever notice versus the in-memory brute-force default, for this repository's actual data volumes (already independently corroborated by Sprint 54's indexed-vs-sequential-scan test).
  - **Latency benchmark: attempted, not completed, confirmed environmental** -- consistent with Phase 4's real-model-download findings (Sprint 49/51), no PostgreSQL instance (via testcontainers/Docker, or any local install) is reachable in this sandbox to measure live. Documented instead, honestly, as expected characteristics rather than fabricated numbers: `InMemoryBookEmbeddingRepository.find_similar()` is an in-process, O(n) brute-force scan with no I/O -- the fastest possible option for this repository's current (demo-scale, single-digit-to-low-hundreds of books) catalog, but it doesn't persist across restarts and doesn't scale to a large catalog. `PostgreSQLBookEmbeddingRepository.find_similar()` adds network round-trip and query-planning overhead per call (making it *slower* than in-memory at this small scale) in exchange for durable, shared, production-appropriate storage and HNSW's approximate O(log n) search, which is what actually matters once the catalog scale changes (Data4Library ingestion, Phase 6, explicitly out of this Phase's scope) -- at that point in-memory search time grows linearly while indexed search stays roughly flat, which is the entire reason this Phase's persistence work exists. This trade-off (small-scale overhead now, essential at production scale) is the honest answer this Sprint can give without a live database to measure against.
  - **Confirmed (again) no ALS/Popularity/Hybrid/ranking-strategy/`SemanticRecommendationEngine` changes**: `git diff --stat` across the entire Phase 5 commit range (`3d921f8~1..HEAD`, i.e. since Phase 4 ended) against those exact files returns empty -- zero lines touched, for the whole Phase, not just this Sprint.
- Validation:
  - `python3 -m ruff check src tests scripts` — pass
  - `python3 -m mypy --strict src tests scripts` — pass (223 source files)
  - `python3 -m pytest -q` — 746 passed (unchanged -- every new test this Sprint requires a live database); 52 pre-existing, Docker-dependent PostgreSQL-test errors (3 more than Sprint 54's count, all this Sprint's own new tests)
...
## Sprint 57 — Import and Normalization Pipeline

### Task 1 — Import and Normalization Pipeline

- Status: Done
- Summary: Reviewed the existing pipeline first (same discipline as every prior Sprint) and found `ImportBooksUseCase` already retrieving/mapping/persisting books with per-record duplicate handling, `book_import_mapper.map_to_book()` already delegating ISBN/Title/Author/Category validation to the domain value objects, and `scripts/import_books.py` already the deliberate orchestration boundary outside the Application layer. The gaps against this Sprint's requirements: duplicates were only ever **skipped**, never **updated** (no real upsert); a record that failed domain validation would raise and abort the whole batch instead of being recorded and skipped (no batch validation); and there was no import history/statistics at all.
  - **Upsert semantics** (`src/readmatch_ai/application/import_books_use_case.py`): for each valid source record, `execute()` now looks up the book by ISBN first. If it already exists, the incoming record's fields are applied via `dataclasses.replace(book, id=existing_book.id)` and persisted via `BookRepository.update()` (preserving the existing `BookId`, so anything already keyed on that identity -- popularity, embeddings -- stays valid); if not, it's persisted via `add()` as before. This is a deliberate behavior change from "skip duplicates" to "upsert," made without asking the user first: the Sprint's own requirement list names "upsert books" explicitly and unambiguously, no REST endpoint exposes import functionality (so no public API contract is affected), and this is the same kind of "the Sprint spec itself settles the design question" judgment call already used in earlier phases (e.g. Sprint 46's reranking policy) -- a routine implementation decision under CLAUDE.md's execution policy, not one requiring architecture change or scope expansion.
  - **Batch validation** (same file): `map_to_book()` (unmodified -- validation logic already lived correctly in the domain value objects) is now called inside a per-record `try/except ValueError`; a record that fails is appended to `ImportBooksResult.invalid_records` (as `f"{isbn13}: {message}"`, so the specific invalid ISBN/title is traceable) and the loop continues -- one malformed record no longer aborts the whole import.
  - **`ImportBooksResult`** redesigned: `imported`/`skipped_duplicate_isbns` (old) became `imported`/`updated`/`invalid_records` (new) -- the old field name no longer describes what happens (duplicates are updated, not skipped), so it was renamed rather than kept as a confusing alias.
  - **Import history** (new): `src/readmatch_ai/domain/import_history.py` defines `ImportHistoryEntry` (imported_at, period_start, period_end, imported_count, updated_count, invalid_count) and the `ImportHistoryRepository` port (`record`/`list_all`); `src/readmatch_ai/infrastructure/in_memory_import_history_repository.py` is the in-memory adapter (matching `book_metadata_repository`'s precedent: always in-memory, no PostgreSQL backend this Sprint -- out of scope). `ImportBooksUseCase` takes a new required `import_history_repository` constructor parameter and records exactly one `ImportHistoryEntry` per `execute()` call, summarizing that run's counts.
  - **Deterministic execution preserved despite a real timestamp requirement**: `ImportHistoryEntry.imported_at` needs real wall-clock time in production but must stay testable/deterministic -- resolved via a `clock: Callable[[], str] = _default_clock` constructor parameter on `ImportBooksUseCase` (defaulting to `datetime.datetime.now(datetime.UTC).isoformat()`), the same dependency-injected-clock pattern already established elsewhere in this codebase for exactly this tension. No import-order/embedding-generation/candidate-ranking logic depends on wall-clock time at all, so "deterministic import behaviour" (this Phase's own architecture constraint) is otherwise unaffected -- the timestamp is metadata attached to the audit entry, never read back by the import logic itself.
  - **`ApplicationContext`** (`src/readmatch_ai/application_context.py`): added `import_history_repository` as a new field, `create()`/`_compose()` parameter, and default-build (`InMemoryImportHistoryRepository()` when not overridden) -- following `book_metadata_repository`'s exact wiring pattern. `ImportBooksUseCase` itself is deliberately **not** constructed inside `ApplicationContext`, preserving the pre-existing, documented boundary (`scripts/import_books.py`'s own docstring): a `BookDataSource` has no sensible default and requires a real external auth key, so import orchestration stays outside the composition root, same as before this Sprint.
  - **`scripts/import_books.py`**: updated to pass `context.import_history_repository` into `ImportBooksUseCase`'s new constructor signature, and its summary print line now reports `imported`/`updated`/`invalid_records` counts instead of the old `imported`/`skipped_duplicate_isbns`.
  - **No embeddings generated, no incremental sync, no scheduler**: confirmed via `git diff --stat` against every embedding-generation and recommendation-engine file -- empty, this Sprint touched only the import pipeline, `ApplicationContext`'s wiring, and this Sprint's own tests.
- Validation:
  - `python3 -m ruff check src tests scripts` — pass
  - `python3 -m mypy --strict src tests scripts` — pass (225 source files)
  - `python3 -m pytest -q tests/application/test_import_books_use_case.py -v` — 10 passed (upsert-within-batch, upsert-against-existing, invalid-record-not-fatal, import-history-recorded-with-injected-clock, import-history-one-entry-per-call, plus the pre-existing popularity-provenance/empty-batch cases updated for the new result shape)
  - **`python3 -m pytest -q` (full suite) — 805 passed, 2 failed.** A real PostgreSQL instance (via Docker) became reachable in this sandbox partway through this Sprint's validation, for the first time across this entire session/Phase 0-6 -- previously-erroring, Docker-dependent tests (Sprints 52-56, `tests/test_import_books_runtime.py` included) now actually execute rather than error. `tests/test_import_books_runtime.py`'s 2 tests (`test_import_runner_persists_and_is_retrievable_via_postgresql`, `test_import_runner_reports_skipped_duplicates`) now run for real and pass, live-validating this Sprint's `scripts/import_books.py` changes end-to-end against a real database. The 2 failures (`test_indexed_and_sequential_retrieval_return_the_same_ranking`, `test_in_memory_and_postgresql_repositories_rank_identically`, both in `tests/infrastructure/test_postgresql_book_embedding_repository.py`) are **confirmed pre-existing and unrelated**: reproduced identically with this Sprint's changes `git stash`-ed out (one of the two still fails against the unmodified Sprint 56 commit; re-running both together is flaky -- different failing subset across the two runs), and both concern HNSW approximate-search ranking from Sprint 54/55, a file this Sprint touched zero lines of. Left unfixed per "do not refactor unrelated code" / "implement only approved Tasks" -- flagged as a known limitation below and in the Phase Completion Report, not silently ignored.
  - `python scripts/run_demo.py` — run twice; diffed with timing/progress-bar lines excluded — identical, confirming determinism is preserved
  - `python scripts/validate_release.py` / `validate_runtime.py` / `validate_deployment.py` — all valid (default `BOOK_REPOSITORY_BACKEND=in_memory` path unaffected by this Sprint's changes)
  - `git diff --stat` confirms zero changes to any embedding-generation or recommendation-engine file (ALS/Popularity/Hybrid/ranking-strategy/`SemanticRecommendationEngine`/embedding generators) -- this Sprint touched only the import pipeline and its own tests
  - `git status`/`git diff` reviewed before staging: only this Sprint's files touched (`scripts/import_books.py`, `import_books_use_case.py`, `application_context.py`, `domain/import_history.py`, `infrastructure/in_memory_import_history_repository.py`, and this Sprint's test files); the pre-existing, unrelated `docs/agent/architecture/*` deletion left untouched and unstaged
- Commit: 6e6d317
- Notes: No architecture change, public-contract break, or destructive operation was required. The skip-to-upsert behavior change is a deliberate, Sprint-spec-directed design decision (see above), not an accidental one. **Known limitation**: the newly-reachable real PostgreSQL instance surfaced a pre-existing, Sprint 54/55-era flaky test pair unrelated to this Sprint's own work (HNSW approximate-search ranking occasionally disagreeing with exact/in-memory ranking) -- out of this Sprint's scope to fix, carried into the Phase Completion Report. This completes Sprint 56-57 / Phase 6 (Data4Library Import Pipeline). Per this Phase's explicit "Out of Scope," embedding regeneration, incremental synchronization, a scheduler, and popularity-update automation remain unimplemented, reserved for later Sprints.

## Sprint 58 — Incremental Synchronization

### Task 1 — Incremental Synchronization

- Status: Done
- Summary: Reviewed the existing pipeline first (same discipline as every prior Sprint) and confirmed the genuine gap: no checkpoint/cursor concept existed anywhere, and `ImportBooksUseCase` only ever reported `imported`/`updated`/`invalid_records` -- a re-imported, byte-identical record was unconditionally written via `update()` and counted as `updated`, and a repository-level failure (as opposed to a validation failure) had no recovery path at all. Data4Library's `loanItemSrch` endpoint has no changed-since parameter (confirmed via the existing connector, unmodified since Sprint 56), so, per this Sprint's own instruction, incremental synchronization is built as deterministic reconciliation by ISBN and normalized content, not a true delta feed.
  - **`ImportBooksUseCase` extended** (`application/import_books_use_case.py`): reconciliation now has three outcomes instead of two -- an existing book whose title/author/category all already match the incoming (normalized, via the same domain VOs as before) values is classified `unchanged` and left untouched (no repository write at all), rather than being unconditionally re-`update()`-d and classified `updated`. A repository-level failure (`BookNotFoundError`/`DuplicateISBNError`, e.g. a race against a concurrent writer) on one record is now caught per-record and recorded in a new `failed_records` list, extending the existing per-record validation resilience to persistence failures -- one bad record still never aborts the batch. `ImportBooksResult` gained `unchanged`/`failed_records`; `ImportHistoryEntry` gained matching `unchanged_count`/`failed_count` fields.
  - **Checkpoint contract** (new): `domain/sync_checkpoint.py` defines `SyncCheckpoint` (period_end, synced_at) and the `SyncCheckpointRepository` port (`get`/`advance`); `infrastructure/in_memory_sync_checkpoint_repository.py` is the required in-memory adapter. A PostgreSQL adapter (`infrastructure/postgresql_sync_checkpoint_repository.py`, single-row table via `migrations/0009_create_sync_checkpoint_table.sql`, `id` fixed to 1 via a CHECK constraint) was added too, since it fits the existing adapter/migration pattern exactly (mirrors `PostgreSQLBookPopularityRepository`'s upsert style) and -- unlike `import_history_repository`, which is audit-only -- a persisted checkpoint is what makes "advance only after success" meaningful across separate process runs, not just within one.
  - **`SynchronizeBooksUseCase`** (new, `application/synchronize_books_use_case.py`): the Sprint's Application-layer synchronization entry point. Wraps an already-constructed `ImportBooksUseCase` (reused unmodified as the one reconciliation engine -- no duplicate import logic was written) and a `SyncCheckpointRepository`. The checkpoint is advanced (to `query.end_date`, timestamped via an injected clock matching `ImportBooksUseCase`'s own convention) only *after* `ImportBooksUseCase.execute()` returns successfully; if the `BookDataSource` itself raises, the exception propagates unchanged and the checkpoint is left untouched.
  - **`ApplicationContext`**: added `sync_checkpoint_repository` (defaults via the same `BookRepositoryConfig.from_env()` switch as `book_repository`/`book_popularity_repository`/`book_embedding_repository`/`user_book_interaction_repository`). `SynchronizeBooksUseCase` itself is not built here, for the same reason `ImportBooksUseCase` isn't (Sprint 57): it needs a `BookDataSource`, which has no sensible default.
  - **`scripts/sync_books.py`** (new): the CLI entry point pairing with `SynchronizeBooksUseCase`, mirroring `scripts/import_books.py`'s structure exactly. Prints created/updated/unchanged/invalid/failed counts plus the checkpoint's new `period_end`.
- Validation:
  - `python3 -m ruff check src tests scripts` — pass
  - `python3 -m mypy --strict src tests scripts` — pass (233 source files)
  - `python3 -m pytest -q` (focused: `test_import_books_use_case.py`, `test_synchronize_books_use_case.py`, `test_in_memory_sync_checkpoint_repository.py`, `test_postgresql_sync_checkpoint_repository.py`, `test_postgresql_book_popularity_repository.py`) — 31 passed, including live PostgreSQL adapter tests (Docker is reachable in this sandbox, confirmed again this Sprint)
  - **Regression found and fixed**: adding `sync_checkpoint_repository` as a fifth `BookRepositoryConfig`-driven repository broke 3 pre-existing tests in `tests/test_application_context.py` (`test_create_supports_a_production_shaped_configuration_using_fakes`, `test_runtime_configuration_summary_never_exposes_the_database_url`, `test_a_fake_repository_override_never_wires_a_persistence_validator`) -- each sets `BOOK_REPOSITORY_BACKEND=postgresql` with a bogus `DATABASE_URL` and explicitly overrides every backend-switching repository with an in-memory Fake to guarantee no real PostgreSQL connection is ever attempted; the new repository wasn't in that override list, so it attempted one and raised `OperationalError`. Fixed by adding `sync_checkpoint_repository=InMemorySyncCheckpointRepository()` to each test's override list, consistent with how the other four backend-switching repositories are already handled there -- not scope creep, but the expected maintenance cost of introducing a fifth one.
  - `python3 -m pytest -q` (full suite) — 819 passed, 2 failed; the 2 failures are the same, already-documented, pre-existing Sprint 54/55 HNSW-ranking flaky pair (`test_indexed_and_sequential_retrieval_return_the_same_ranking`, `test_in_memory_and_postgresql_repositories_rank_identically`), untouched by this Sprint
