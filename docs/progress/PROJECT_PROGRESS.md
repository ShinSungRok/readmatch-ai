...
- Current Sprint: Sprint 29 — Explainable Recommendations — Complete
- Last Completed Task: Sprint 29 / Task 1 — Explainable Recommendations
- Last Commit: (recorded after commit; Sprint 29 / Task 1)
- Validation: Established — `ruff check`, `mypy` (strict), `pytest` all passing (357 tests, up from 319, ~86s including PostgreSQL/testcontainers suites); `scripts/run_demo.py` re-run directly (3x, confirmed deterministic reason output each time); `GET /recommendations/personalized/{user_id}/explained` smoke-tested directly and confirmed in the OpenAPI schema
